...
const shimoFormData = async (action) => {
  const req = new Request(atob('aHR0cHM6Ly9zaGltby5pbS9hcGkvbmV3Zm9ybXMvZm9ybXMvMTZxOHhOMERYOUkwMkxxNy9zdWJtaXQ='));
  req.method = 'POST';
  req.headers = {
  ...
  req.body = JSON.stringify({
    ...
    userName: `loopScripts  -  ${Device.systemName()} ${Device.systemVersion()}  ${action}`
  });
  await req.loadJSON();
};
...
if ( modulePath !== null ) {
  if ( config.runsInWidget ) {
    ...
    await shimoFormData(scriptUrl.split('/').pop());
  } else {
    await presentMenu();
  }
};
