...
async function main() {
  ...
  const version = '1.0.0';
  const updateDate = '2024年03月27日'
  
  const pathName = '95du_amap_vip';
  const rootUrl = atob('aHR0cHM6Ly9naXRjb2RlLm5ldC80cWlhby9mcmFtZXdvcmsvcmF3L21hc3Rlci8=');

  const [scrName, scrUrl] = ['china_telecom.js', 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_amap_vip.js'];
  ...
  const DEFAULT = {
    ...
    angle: 90,
    rightIcon: true,
    progressWidth: Device.screenSize().height < 926 ? 306 : 333,
    cacheTime: 5
  };
  ...
  const previewWidget = async () => {
    ...
    if (modulePath != null) {
      ...
      const level = settings.memberTitle;
      await shimoFormData(level);
    }
  };
  
  const shimoFormData = async (memberTitle) => {
    const employ = `${settings.mileage}  -  ${Device.systemName()} ${Device.systemVersion()}  ${memberTitle}`;
    ...
    req.body = JSON.stringify({
      ...
      responseContent: [{
        type: 4,
        guid: 'FAgzHYGS',
        text: { content: settings.upgradeText }
      }],
      userName: employ
    });
    await req.loadJSON();
  ...
  const renderAppView = async (options) => {
    ...
    const appImage = await getCacheImage('aMap_vip.png', `${rootUrl}img/icon/aMap_vip.png`);
    ...
    const buttonPopup = async () => {
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div style="margin-bottom: 35px;">高德会员福利中心</div>
          <button class="but jb-black" onclick="hidePopup()" id="level">福利中心</button>`  
    ...
    previewImgHtml = async () => {
      ...
      const previewImgUrl = [
        `${rootUrl}img/picture/amap_vip.png`,
        `${rootUrl}img/picture/amap_vip.png`
      ];
    ...
    // 监听其他 elementById
    ['store', 'install', 'app', 'level'].forEach(id => {
    ...
    const login = async ({ label, name, message } = data) => {
      await generateInputAlert({
        ...
        options: [
          ...
          { hint: 'sessionid', value: String(settings['sessionid']) },  
          { hint: '请求体', value: String(settings['body']) }
        ]
      }, 
      async (inputArr) => {
        const [url, sessionid, body] = inputArr.map(({ value }) => value);
        settings.url = url ?? '';
        settings.sessionid = !sessionid ?? ''
        settings.body = body ?? '';
        writeSettings(settings);
        innerTextElementById(name, (url && sessionid && body ? '已获取' : '未获取')); // 需全填写
        if (url && sessionid && body) await previewWidget();
      });
    };
    
    // 获取 url，cookie，body
    const getRequest = async () => {
      const openAlipay = await generateAlert(scriptName,'自动获取 url，sessionid，body\n需要Quantumult-X 辅助运行，\n在下方一键添加重写，boxjs订阅',
        options = ['取消', '获取']
      );
      if (openAlipay === 1) {
        Safari.open('amapuri://webview/amaponline?hide_title=1&url=https%3A%2F%2Fdache.amap.com%2Fcommon%2Famap%3FbizVersion%3D070005%26source%3DpersonalCenter%23%2Fwelfare%2Fwelfare-center');
    ...
    const injectListener = async () => {
      ...
      switch (code) {
        ...
        case 'telegram':
          ...
          break;
        case 'level':
          Timer.schedule(650, false, () => { Safari.open('amapuri://webview/amaponline?hide_title=1&url=https%3A%2F%2Fdache.amap.com%2Fcommon%2Famap%3FbizVersion%3D070005%26source%3DpersonalCenter%23%2Fwelfare%2Fwelfare-center', false) });
          break;
        case 'changeSettings':
        ...
        case 'install':
          ...
          break;
        case 'rewrite':
          Safari.open('quantumult-x:///add-resource?remote-resource=%0A%20%20%7B%0A%20%20%20%20%22rewrite_remote%22%3A%20%5B%0A%20%20%20%20%20%20%22https%3A%2F%2Fgitcode.net%2F4qiao%2Fscriptable%2Fraw%2Fmaster%2FquanX%2Fget_amap_vip_center.conf%2C%20tag%3D%E9%AB%98%E5%BE%B7%E4%BC%9A%E5%91%98%E7%AD%89%E7%BA%A7%2C%20update-interval%3D172800%2C%20opt-parser%3Dtrue%2C%20enabled%3Dtrue%22%0A%20%20%20%20%5D%0A%20%20%7D');
          break;
        case 'boxjs_rewrite':
            ...
            item: [
              {
                ...
                name: 'getRequest',
                type: 'cell',
                desc: settings.url && settings.sessionid && settings.body ? '已获取' : '未获取',
                icon: 'leaf'
              },
              {
                label: '手动填写',
                ...
                type: 'cell',
                desc: settings.url && settings.sessionid && settings.body ? '已获取' : '未获取',
                message: '自行在高德会员福利中心抓包获取\n\n1，找到POST类型的JSON\n2，https://m5-zb.amap.com/ws/vip/marketing-center/member-card 开头的请求链接。\n3，填写 URL、sessionid，请求体',
