...
const getCacheImage = async (name, url) => {
  const cache = useFileManager({ cacheTime: 1240 });
...
const getWeather = async (loc) => {
  try {
    const convert = await getJson(`https://restapi.amap.com/v3/assistant/coordinate/convert?coordsys=gps&output=json&key=979aa069d4974f4994bd85fd13d273d0&locations=${loc.longitude},${loc.latitude}`);
    const [lon, lat] = convert.locations.split(",");
    ...
    const { radarData: { title, content } } = await request.loadJSON();
    return { title, content, longitude: lon, latitude: lat };
  } catch (e) {
    console.log(e + '⚠️使用缓存');
...
const getIcon = async () => {
  const images = [
    'https://raw.githubusercontent.com/95du/scripts/master/img/icon/weChat.png',
    'https://raw.githubusercontent.com/95du/scripts/master/img/icon/weather.png'
  ]
...
const createWidget = async () => {
  let { title, content, longitude, latitude } = await getWeather(await getLocation());
