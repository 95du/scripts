async function main() {
  ...
  const [scrName, scrUrl] = [`12123_${updateDate}.js`, 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_12123_4.js'];
  ...
  const renderAppView = async (options) => {
    ...
    const scriptTags = await Promise.all(scripts.map(async (script) => {
      const content = await getCacheString(script, `${rootUrl}web/${script}?ver=8.0`);
        ...
        if (item.type === 'group') {
          ...
          collapsible.querySelectorAll('.form-item').forEach((label, index) => {
            label.addEventListener( 'click', () => {
              ...
              if (labelId === 'referer') switchDrawerMenu();
