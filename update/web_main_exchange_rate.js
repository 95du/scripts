async function main() {
  ...
  const version = '1.0.0'
  const updateDate = '2023年11月04日'
  
  const pathName = '95du_exchange_rate';
  const rootUrl = 'https://gitcode.net/4qiao/framework/raw/master/';

  const [scrName, scrUrl] = ['exchange_rate.js', 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_exchange_rate.js'];
  ...
  const getSettings = (file) => {
    if (fm.fileExists(file)) {
      return { currency } = JSON.parse(fm.readString(file));
    } else {
    ...
    const buttonPopup = async () => {
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div style="margin-bottom: 30px;">常用国际货币兑换人民币</div>
