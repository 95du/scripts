async function main() {
  const scriptName = '京东收支账单'
  const version = '1.0.1'
  const updateDate = '2023年11月11日'
  const pathName = '95du_jd_Bill';
  const rootUrl = 'https://gitcode.net/4qiao/framework/raw/master/';

  const [scrName, scrUrl] = ['jingDong_bill.js', 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_jingDong_bill.js'];
  ...
  const DEFAULT = {
    ...
    progressWidth: screenSize < 926 ? 215 : 243,
    progressHeight: screenSize < 926 ? 9 : 10,
    randomIndex: 0,
    statistics: 1,
    cacheTime: 5
  };
  ...
  const shimoFormData = (action) => {
    const req = new Request('https://shimo.im/api/newforms/forms/XKq42DPGbrhVLNAN/submit');
    req.method = 'POST';
    req.headers = {
  ...
  if (config.runsInWidget) {
    ...
    if (version !== settings.version && !settings.update && hours >= 12) {
      ...
      writeSettings(settings);
      notify(`${scriptName}‼️`, `新版本更新 Version ${version}，增加缓存数据，减少网络请求`, 'scriptable:///run/' + encodeURIComponent(Script.name()));
    };
  ...
  const renderAppView = async (options) => {
    ...
    const scriptTags = await Promise.all(scripts.map(async (script) => {
      const content = await getCacheString(script, `${rootUrl}web/${script}?ver=7.7.9`);
    ...
    const buttonPopup = async () => {
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div style="margin-bottom: 30px;">京东收支账单</div>
    ...
    // 监听其他 elementById
    ['store', 'install', 'app', 'alipay'].forEach(id => {
    ...
    const webLogin = async ({ label, name } = data) => {
      ...
      if (req.response.statusCode === 200 && cookies.length) {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            type: 'color',
            input: true,
            icon: `${rootUrl}img/symbol/progressColor1.png`,
            message: '支出进度条颜色 ( 输入Hex颜色代码 )',
            desc: settings.progressColor1
          },
          {
            label: '收入进度',
            name: 'progressColor2',
            type: 'color',
            input: true,
            icon: `${rootUrl}img/symbol/progressColor2.png`,
            message: '收入进度条颜色 ( 输入Hex颜色代码 )',  
            desc: settings.progressColor2
          }
        ]
      },
      {
