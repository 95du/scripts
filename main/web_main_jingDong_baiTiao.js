...
async function main() {
  ...
  const version = '1.1.0'
  const updateDate = '2024年10用24日'
  const pathName = '95du_jd_baiTiao'
  
  const rootUrl = 'https://raw.githubusercontent.com/95du/scripts/master';
  const [scrName, scrUrl] = ['baiTiao.js', `${rootUrl}/api/web_jingDong_baiTiao.js`];
  /**
   * 创建，获取存储路径
  ...
  const shimoFormData = (action) => {
    const req = new Request('https://shimo.im/api/newforms/forms/5xkGozaxxXFOQMkX/submit');
    req.method = 'POST';
    req.headers = {
  ...
  const renderAppView = async (options) => {
    ...
    const buttonPopup = async () => {
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div style="margin-bottom: 30px;">京东白条、等级</div>
    ...
    previewImgHtml = async () => {
      ...
      const pictureArr = Array.from({ length: 2 }, (_, index) => `${rootUrl}/img/picture/jingDong_baiTiao_${index}.png`);
      const randomImageUrl = pictureArr[Math.floor(Math.random() * pictureArr.length)];
      
      const previewImgUrl = [
        randomImageUrl,
        `${rootUrl}img/picture/jingDong_baiTiao_2.png`
      ];
