...
async function main() {
  ...
  const version = '1.0.0'
  const updateDate = '2024年04月06日'

  const pathName = '95du_china_unicom';
  const rootUrl = atob('aHR0cHM6Ly9naXRjb2RlLm5ldC80cWlhby9mcmFtZXdvcmsvcmF3L21hc3Rlci8=');
  
  const [scrName, scrUrl] = ['china_unicom.js', 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_china_unicom.js'];
  ...
  const renderAppView = async (options) => {
    ...
    const appImage = await getCacheImage('china_unicom.png', `${rootUrl}img/icon/china_unicom.png`);
    ...
    .modal-dialog {
      ...
      top: ${screenSize < 926 ? (avatarInfo ? '-3.5%' : '-2%') : (avatarInfo ? '-7.5%' : '-4%')};
    }
    ...
    const buttonPopup = async () => {
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div style="margin-bottom: 30px;">中国联通</div>
          <button class="but jb-red" onclick="hidePopup()" id="cookie">获取 Cookie</button>`  
