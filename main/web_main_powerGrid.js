...
async function main() {
  ...
  const version = '1.0.1'
  const updateDate = '2024年05月06日'
  
  const pathName = '95du_powerGrid';
  const rootUrl = atob('aHR0cHM6Ly9naXRjb2RlLm5ldC80cWlhby9mcmFtZXdvcmsvcmF3L21hc3Rlci8=');

  const [scrName, scrUrl] = ['powerGrid_2.js', 'https://gitcode.net/4qiao/scriptable/raw/master/table/web_powerGrid_2.js'];
  ...
  const DEFAULT = {
    ...
    titleColor: '#000000',
    rangeColor: '#3F8BFF',
    progressWidth: screenSize < 926 ? 218 : 248,
    gap: screenSize < 926 ? 15 : 20,
    location: 1,
    radius: 50,
    avatar: 'https://gitcode.net/4qiao/scriptable/raw/master/img/icon/zhuYuanZhang.jpeg'
  };
  ...
  const renderAppView = async (options) => {
            ...
            <div class="tab-content">
              ...
              <div class="tab-pane fade active in" id="tab-sign-in">
                <div class="box-body">
                  ...
                  <div class="form-label-title update-desc"> <li>${updateDate}</li> <li>修复已知问题</li> <li>增加多账号循环显示</li> <li>性能优化，改进用户体验</li>
                  </div>
                </div>
    ...
    const buttonPopup = async () => {
      ...
      const content = `${avatarInfo  
        ? `<img id="app" onclick="switchDrawerMenu()" class="app-icon" src="${appImage}">
          <div style="margin-bottom: 35px;">南网在线 App ( 5 省 )</div>
    ...
    previewImgHtml = async () => {
      ...
      const pictureArr = Array.from({ length: 2 }, (_, index) => `${rootUrl}img/picture/powerGrid_${index}.png`);
    ...
    const userlogin = async ({ label, name } = data) => {
      ...
      if ( openAlert === 1 ) {
        ...
        const input = await generateInputAlert({
